...
@set str_dpl=.\%war_ear_common_prefix%* -x!.\*.war.* -x!.\*.ear.*
@set str_marker=..\*.dodeploy
@if not "%~1"=="" (set str_dpl=.\%~1\%war_ear_common_prefix%* -x^!.\%~1\*.war.* -x^!.\%~1\*.ear.*)
...
@pushd %deploy_dir%
@for /D %%f in (%war_ear_common_prefix%*) do @copy NUL %jboss_deploy_dir%\..\%%f.dodeploy >NUL
